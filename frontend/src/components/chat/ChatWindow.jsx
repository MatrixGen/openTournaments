...
export default function ChatWindow({ tournament, channel }) {
  ...
  const handleSend = useCallback(
    async (content, mediaFile = null, options = {}) => {
  ...
  if (!chatUser && isLoading) {
    return (
      <div
        className={`flex items-center justify-center h-full ${themeClasses.container}`}
      >
  ...
  if (chatError) {
    return (
      <div
        className={`flex flex-col items-center justify-center h-full ${themeClasses.container} p-4`}
      >
        <div className="text-red-500 mb-2">⚠️ Connection Error</div>
        <div className="text-gray-500 text-center mb-4">{chatError}</div>
        <button
          onClick={() => window.location.reload()}
          className="px-4 py-2 bg-blue-500 text-gray-900 dark:text-white rounded-lg hover:bg-blue-600"
        >
          Retry Connection
        </button>
      </div>
  ...
  return (
    <div
      className={`flex flex-col h-full ${themeClasses.container} rounded-lg md:rounded-xl overflow-hidden`}
    >
      {/* Header */}
      <ChatHeader
