...
import { 
  ArrowPathIcon,      // ✅ exists
  Squares2X2Icon,     // ✅ exists
...
// Compact match component for mobile
const MatchNode = memo(({ match, isHighlighted = false }) => {
  ...
  return (
    ...
    } p-3 min-w-[180px] transition-all duration-200 hover:shadow-md`}>
      {/* Match header */}
      <div className="flex justify-between items-center mb-2">
        {isLive && (
          <span className="flex items-center gap-1 text-xs font-medium px-2 py-0.5 rounded bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400">
            <span className="w-1.5 h-1.5 rounded-full bg-red-500 animate-pulse"></span>
            LIVE
          </span>
        )}
        {isCompleted && !isLive && (
          <span className="text-xs font-medium px-2 py-0.5 rounded bg-green-100 dark:bg-green-900/30 text-green-600 dark:text-green-400">
            DONE
          </span>
        )}
        {match.match_order && (
...
const BracketVisualization = memo(({ rounds, viewMode, onScrollLeft, onScrollRight }) => {
    ...
    <div className="bg-white dark:bg-neutral-800 rounded-lg border border-gray-200 dark:border-neutral-700">
      ...
      <div className="p-4 border-b border-gray-200 dark:border-neutral-700">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <ArrowPathIcon className={`h-5 w-5 text-gray-600 dark:text-gray-400 ${isRefreshing ? 'animate-spin' : ''}`} />
            </button>
      ...
      <div className="lg:hidden px-4 pb-4">
        <p className="text-xs text-gray-500 dark:text-gray-400 text-center">
          Swipe horizontally to view the entire bracket
        </p>
      </div>
    </div>
  );
