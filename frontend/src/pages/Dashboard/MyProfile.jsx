...
const StatCard = ({ label, value, icon: Icon, color, bgColor, suffix }) => (
  <div className={`bg-white dark:bg-neutral-800 rounded-lg md:rounded-xl p-4 border border-gray-200 dark:border-neutral-700 hover:border-gray-300 dark:hover:border-neutral-600 transition-colors`}>
    <div className="flex items-center justify-between">
      <div>
        <p className="text-gray-600 dark:text-gray-400 text-xs md:text-sm font-medium mb-1">{label}</p>
    ...
    <div className="flex items-center space-x-3">
      ...
      <div className="flex-1">
        <h4 className={`font-medium text-sm ${
          unlocked ? 'text-gray-900 dark:text-white' : 'text-gray-600 dark:text-gray-400'
        }`}>
          {title}
        </h4>
...
export default function MyProfile() {
  ...
  const loadUserData = async () => {
    ...
    try {
      ...
      const tournamentsResponse = await userService.getUserTournaments({ limit: 3 });
  ...
  const statCards = [
    {
      ...
      icon: TrendingUp,
      color: userStats?.win_rate >= 50 ? 'text-green-600 dark:text-green-400' : 'text-yellow-600 dark:text-yellow-400',
      bgColor: userStats?.win_rate >= 50 ? 'bg-green-100 dark:bg-green-900/30' : 'bg-yellow-100 dark:bg-yellow-900/30',
      suffix: '%'
    },
    {
      label: 'Matches Won',
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-neutral-900 safe-padding">
      <main className="mx-auto max-w-4xl py-4 md:py-6 px-3 sm:px-4 lg:px-8 space-y-4 md:space-y-6">
        ...
        <div className="bg-white dark:bg-neutral-800 rounded-xl shadow border border-gray-200 dark:border-neutral-700 p-4 md:p-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-4 mb-4">
            <div className="bg-gray-50 dark:bg-neutral-700/50 rounded-lg md:rounded-xl p-3 md:p-4">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-gray-600 dark:text-gray-400 text-xs md:text-sm font-medium mb-1">Wallet Balance</p>
            ...
            <div className="bg-gray-50 dark:bg-neutral-700/50 rounded-lg md:rounded-xl p-3 md:p-4">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-gray-600 dark:text-gray-400 text-xs md:text-sm font-medium mb-1">Member Since</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
          ...
          <div className="bg-white dark:bg-neutral-800 rounded-xl shadow border border-gray-200 dark:border-neutral-700 p-4 md:p-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg md:text-xl font-semibold text-gray-900 dark:text-white">Detailed Stats</h2>
              <BarChart className="h-5 w-5 text-gray-400" />
            </div>
            <div className="space-y-3">
              <div className="flex justify-between items-center py-2 border-b border-gray-100 dark:border-neutral-700">
                <span className="text-gray-600 dark:text-gray-400 text-sm md:text-base">Tournaments Joined</span>
                <span className="text-gray-900 dark:text-white font-bold text-sm md:text-base">
                  {userStats?.tournaments_joined}
                </span>
              </div>
              <div className="flex justify-between items-center py-2 border-b border-gray-100 dark:border-neutral-700">
                <span className="text-gray-600 dark:text-gray-400 text-sm md:text-base">Highest Win</span>
                <span className="text-emerald-600 dark:text-emerald-400 font-bold text-sm md:text-base">
                  {formatCurrency(userStats?.highest_win)}
                </span>
              </div>
              <div className="flex justify-between items-center py-2 border-b border-gray-100 dark:border-neutral-700">
                <span className="text-gray-600 dark:text-gray-400 text-sm md:text-base">Avg. Position</span>
                <span className="text-gray-900 dark:text-white font-bold text-sm md:text-base">
                  #{userStats?.average_position }
                </span>
              </div>
              <div className="flex justify-between items-center py-2">
                <span className="text-gray-600 dark:text-gray-400 text-sm md:text-base">Best Streak</span>
          ...
          <div className="bg-white dark:bg-neutral-800 rounded-xl shadow border border-gray-200 dark:border-neutral-700 p-4 md:p-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg md:text-xl font-semibold text-gray-900 dark:text-white">Account Status</h2>
              <Shield className="h-5 w-5 text-gray-400" />
            </div>
            <div className="space-y-3">
              <div className="flex justify-between items-center py-2 border-b border-gray-100 dark:border-neutral-700">
                <span className="text-gray-600 dark:text-gray-400 text-sm md:text-base">Verification</span>
                <span className={`font-bold text-sm md:text-base ${
                  user?.is_verified ? 'text-green-600 dark:text-green-400' : 'text-yellow-600 dark:text-yellow-400'
                }`}>
                  {user?.is_verified ? 'Verified' : 'Pending'}
                </span>
              </div>
              <div className="flex justify-between items-center py-2 border-b border-gray-100 dark:border-neutral-700">
                <span className="text-gray-600 dark:text-gray-400 text-sm md:text-base">Account Role</span>
                <span className="text-primary-600 dark:text-primary-400 font-bold text-sm md:text-base capitalize">
                  {user?.role || 'Player'}
                </span>
              </div>
              <div className="flex justify-between items-center py-2 border-b border-gray-100 dark:border-neutral-700">
                <span className="text-gray-600 dark:text-gray-400 text-sm md:text-base">Total Deposits</span>
                <span className="text-gray-900 dark:text-white font-bold text-sm md:text-base">
                  {formatCurrency(userStats?.total_deposits || 0)}
                </span>
              </div>
              <div className="flex justify-between items-center py-2">
                <span className="text-gray-600 dark:text-gray-400 text-sm md:text-base">Last Active</span>
        ...
        <div className="bg-white dark:bg-neutral-800 rounded-xl shadow border border-gray-200 dark:border-neutral-700 p-4 md:p-6">
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2 mb-4">
            <div>
              <h2 className="text-lg md:text-xl font-semibold text-gray-900 dark:text-white">Recent Tournaments</h2>
              <p className="text-gray-600 dark:text-gray-400 text-sm mt-1">
                Your latest tournament activity
              </p>
            </div>
            <Link to="/my-tournaments" className="inline-flex items-center text-primary-600 dark:text-primary-500 hover:text-primary-700 dark:hover:text-primary-400 text-sm font-medium">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className={`px-2 py-1 rounded-full text-xs ${
                          ...
                          tournament.status === 'active' ? 'bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-400' :
                      ...
                      <div className="flex items-center gap-3 text-gray-600 dark:text-gray-400 text-xs">
                        ...
                        <span>•</span>
                        <span>Entry: {formatCurrency(tournament.entry_fee || 0)}</span>
                      </div>
                    </div>
                    <div className="text-right">
                      {tournament.position && (
                        <p className={`font-medium text-sm md:text-base ${
                          tournament.position === 1 ? 'text-yellow-600 dark:text-yellow-400' :
                          tournament.position <= 3 ? 'text-emerald-600 dark:text-emerald-400' :
